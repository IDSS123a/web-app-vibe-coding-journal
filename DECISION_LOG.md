...
**Design** (`specs/prompt-school/`). Twelve chapters in three levels mapped to the book (beginner: craft, five pillars, foundational techniques, formatting; intermediate: reasoning, structure and protection, code and research, optimize and debug; advanced: ethics and bias, tools and multimodal, case studies, blueprint workshops). Content is authored text stored in the database by an idempotent seed script (`scripts/seed-prompt-school.ts`), so the free AI quota is untouched (PDL-021, PDL-058). Five exercise kinds, all graded on the server without AI: choice, fill in the blanks, order the parts (Up and Down buttons, no drag), spot the flaw, repair the prompt (a rubric of alternative patterns per criterion, weighted; feedback names each missed criterion with a hint). An exercise passes at 75 percent, a chapter at an average best score of 75 percent over all its exercises.

**Unlocking (Director, 2026-09-20: "same logic as the University by chapters, the next opens only when the previous is finished").** Practice opens when every lesson of the chapter is marked done (the University's "quiz available" rule); a chapter is complete when its practice is passed at 75 percent; the next chapter opens only when the previous one is complete. The rule lives in one pure function (`isChapterUnlocked`) and one state builder (`features/prompt-school/progress.ts`) used by the overview and by every route, which answer 403 for a locked chapter, its lessons, and grading before the lessons are done. Because "The Five Pillars" is book chapter 2 and chapter 1 is not written yet, "previous" means the previous PUBLISHED chapter, and a chapter the learner has already started stays open, so publishing chapter 1 later locks nobody out of work in progress. The exercises are not even listed to the browser until the practice is open. Verified end to end in the probe with a temporary second chapter: locked while chapter one is incomplete, open once complete, locked again if chapter one falls below 75 percent and nothing was started in it, temp data removed again. Correct answers, rubrics and explanations never leave the server before an attempt: the chapter route returns only the public half (`toPublicExercise`), and the probe checks the payload for any answer field. Access is the University rule (`canAccessPromptSchool`, admin exempt, blocked and expired refused); Basic and trial users get the upsell screen (`PremiumPitch` feature "promptschool") with checkout on the same screen. Tables `ps_*` (migration 030) are default deny like every table since migration 022.
...
**Verification:** 274 unit tests (45 new: grading, validation, content, permissions, unlocking), typecheck, lint, security probe 130 checks (was 96: anonymous 401 on all five routes, tier matrix premium, basic, trial, ended, expired and blocked for the overview and the grading route, bad slugs, non-uuid ids, wrong-shaped answers, missing body, no answer fields in the chapter payload, the unlocking walk above), end to end smoke 30 checks in real Chrome (pages, lesson completion, one exercise graded by the server, one rubric graded, the test account's progress removed afterwards), responsive audit 276 checks in 12 viewport settings (280 to 3440 px, OS dark mode), 23 pages of which 4 are new, 0 issues.

**Not built yet (phases in `specs/prompt-school/TASKS.md`):** the other eleven chapters (about 41 lessons), level tests, capstone workshops from Appendix B, the live sandbox (3 runs a day, shares the free AI pool), coins and badges. Level tests (like the University's) are not built yet.
...
**Finding for the Director (manuscript).** In the manuscript file and in the corrected .docx, the detailed text of chapter 9 sections 1 and 2 is replaced by an editorial placeholder, and chapter 10's second case study points at "the previous response" for three templates (the review assistant V1, the methodology extractor, the stated limitations finder). The approved text exists only in `Prirucnik_Prompt_Engineering_B.docx`, and the lessons teach from it for those passages. The manuscript should be repaired so that the book itself contains what the School teaches. Update the same day: the Director declared Manual_Prompt_Engineering_10042025_ORIGINAL_WITH_COVER-CORRECTIONS-28122025.docx the CANON (last version, wins any conflict) and asked for a completeness check. Result: Mastering_Prompt_Engineering.md is a faithful export of the canon (a sentence-level comparison in both directions found only formatting differences and no missing content), so the School was read from the right text. The canon itself has the same five holes (chapter 9 sections 1 and 2 including the Museum Case Study, and the three templates of chapter 10's second case study). It also holds four stray drafting notes in parentheses (the chapter 2 BiH workshop, the chapter 4 Anya case study, which even contains an AI-voice remark, the chapter 6 climate example and the chapter 7 blog workshop), a bracketed leftover in the Collaborative Markdown section, and chapter cross-references that use the old numbering from before the Craft chapter became chapter 1 (for example "Structured Output (Chapter 4)" where the chapter is now 5) in the chapters, in Appendix B and in Appendix D, while the glossary uses the new numbering. The lessons therefore name techniques instead of chapter numbers.
...
## PDL-069 Prompt School: Appendix B, the fifteen blueprints as three workshops

**Date:** 2026-09-20. Director: after chapters 8 to 11, continue with Appendix B, 15 templates in three workshops. Appendix B (about 2,800 lines of the book) was read in full before anything was written.

- **Structure.** Every blueprint in the book has the same four parts: the scenario with its detailed explanation (domain, scenario, goal, objective, techniques used and why, intended achievement), the Markdown prompt blueprint, the flowchart, and the suggested next steps (prompt refinement, output application, integration). The book map now lists all four parts of every blueprint as separate sections, so a lesson cannot leave one out, and `content/blueprint-lesson.ts` builds each blueprint lesson from one structured record so all fifteen read the same way. The complete prompt of each blueprint is kept whole in a code block, because it is the reusable asset. A flowchart cannot be drawn in a lesson, so its logic is given as numbered steps in reading order.
- **Workshop 1** (`blueprints-1`, 6 lessons, 10 exercises, 21 sections): how to use the library (select, understand, customize, test), then perfume launch analysis, warehouse dispatch, shift-based production material flow, banking customer analysis under strict ethics, and the educational lesson plan.
- **Workshop 2** (`blueprints-2`, 5 lessons, 10 exercises, 20 sections): retail replenishment and shelf allocation, pharmaceutical market analysis, call center quality assurance, personal finance guidance for an authenticated user, and the interactive to-do list.
- **Workshop 3** (`blueprints-3`, 6 lessons, 10 exercises, 21 sections): the language learning center, laboratory sample tracking, fitness client assessment, investigative journalism assistance, scientific paper writing, and the closing "From Blueprints to Bespoke Tools" (customization, contextualization, refinement, ethical scrutiny), which also names what all fifteen share.
- **Exercises** follow the same rules as every chapter: five kinds, server-side grading, repair exercises with a tested rubric (dispatch request, call QA prompt, scientific discussion prompt), and calculation exercises a learner can check by hand (which task is overdue on the book's date).
- **Source fidelity.** The blueprint text follows the canon document. Its chapter numbers in the justifications ("Structured Output (Chapter 4)") are off by one and were not copied.

The School stands at 86 lessons in 14 chapters, 276 of 297 book sections (93 percent). What remains is Appendix D (techniques quick reference), Appendix A (glossary) and Appendices E and F (resources and platforms). Verified: 163 Prompt School tests (399 in the whole project), lint and typecheck clean, integrity pass, content seeded to production, responsive audit on every lesson page at 280 px with no overflow. Not yet pushed: the Director has approved pushes up to chapter 11.
...
## PDL-087 The Daily Report stopped generating for three days, and why

**Date:** 2026-09-25. The Director asked whether her admin login felt slow; timed, real requests against both domains (login, `/api/me`, the post-login pages) came back at 0.3 to 1.2s, nothing reproduced it. Investigating anyway turned up something real and much bigger.

**Found.** No Daily Report had been created since 2026-09-22, three days, not held for review, simply never generated, while 991 relevant, well-scored articles sat waiting (some scored as recently as two hours before this was found, at 90+). `check:integrity`'s data was fine; this was a live pipeline stall the existing tests had no way to see. `ai_call_log` showed only 13 AI calls total in the last 24 hours, against a normal day's 50 to 150+, and the newest article with a summary was timestamped 2026-09-19T13:55 -- summarisation had not produced a single result since that exact moment. Calling `summarize()` directly, right now, with real content: it worked perfectly on the first try. So the function was not broken; it was not getting a turn.

**Root cause.** Sources grew from 14 to 30 on 2026-09-19 (PDL-059), and `SOURCE_HEALTH_CONFIG.HTTP_TIMEOUT_MS` (`features/sources/domain.ts`) was, and had been since it was introduced, `8000`, not the `5000` its own comment's math already assumed -- the comment and the code had quietly disagreed since before this session ever touched the file. At the real 8000, a single source can cost up to two sequential 16 second network legs (reachability, then the feed fetch) before `SOURCE_PROCESSING_BUDGET_MS` (`features/sources/actions.ts`, an outer 20s ceiling wrapping both) even applies. With `POLL_CONCURRENCY` at 4 and 30 sources, the worst case for collection alone (Phase 1) grew past two and a half minutes, once several real sources (Replit Blog, Cloudflare Blog, Simon Willison, confirmed in the runtime error logs) actually hit that ceiling. Vercel's function budget is about 300s total; whatever Phase 1 spent was gone for Phase 3, the enrichment step that turns a relevant article into a reportable one with an AI summary -- and it kept losing that race, silently, run after run, for six days.

**A second, smaller bug compounded it.** `getRelevantUnfinishedArticles` (`features/pipeline/repository.ts`, "queue 2", the retry list for relevant-but-not-yet-summarised articles) filtered on `confidence_score IS NULL`. `runEnrichmentCycle` (`features/pipeline/enrichment.ts`) writes `confidence_score` (a local heuristic, no AI call, cannot itself fail) BEFORE calling `summarize()` in the same try block -- so the moment `summarize()` failed for any reason, that article's `confidence_score` was already committed, and queue 2 would never offer it again. 71 articles were permanently orphaned this way: scored, confident, never summarised, invisible to their own retry queue. Fixed to filter on `summary IS NULL` instead, which is what "not finished" actually means; this alone put all 71 back in play. (A near-identical pattern exists in `getNonDuplicateArticles`'s own `confidence_score IS NULL` check, for a different queue and an older, unrelated 2026 incident -- not touched here, flagged for a later look.)

**Fix, the Director's chosen option (shorten the per-source wait).** `HTTP_TIMEOUT_MS` 8000 -> 5000 (matching what the existing comment already claimed), `SOURCE_PROCESSING_BUDGET_MS` 20000 -> 12000 (kept safely above the now-real 2*5000=10s network floor, not an independent guess). New worst case for the whole collection phase, 30 sources: about 104s, down from up to 168s -- comfortably inside the 300s budget with real time left for enrichment every run, not just on a good one.

**Separately, found while reading the logs: `RESEND_API_KEY` has answered "API key is invalid" (401) since 2026-09-22T04:19** -- every admin payment/report notification since then has silently failed to send, and so has every health-check alert (PDL-081) since the day after it shipped, which is exactly why none of this reached the Director before she noticed it herself. Needs a new key from her (Resend dashboard, same steps as `LAUNCH_GUIDE.md` 2d), not something fixable from here.

**Verified.** Typecheck, lint, 517 unit tests. Live: `summarize()` called directly with real content, succeeds. The two constants and the queue-2 filter deployed, then the production cron endpoint was triggered directly (same call the scheduled GitHub Action makes) to confirm real recovery rather than trust the math alone.
...
## PDL-090 The real reason nothing was ever summarised, even with 10 keys: a stuck key never rotated away

**Date:** 2026-09-26. The Director reported three things: University lesson generation taking almost a minute, no Daily Report since 2026-09-22 (four days), and a password-reset e-mail to `mulalic71@gmail.com` never arriving. The first two turned out to be the same bug; the third is unrelated.

**No report since 09-22, confirmed live.** Queried the database directly: 1,233 articles were relevant and scored but had no summary, growing by the hour; the last successful `summarize()` call anywhere in the system was 2026-09-21T03:38, five days earlier. Triggered the production cron directly to watch a real run: source collection now takes 29.8s (PDL-087's fix holding up), but quality scoring took the full 145s it was allowed and still produced `AI-summarized: 0`. The run's own log showed the actual mechanism: `[GEMINI] key 1 rate-limited, trying key 2` -- and then silence for the rest of the 145 seconds. Only two key-rotation attempts logged in over two minutes, no matter that ten keys are now configured.

**Root cause.** `callGeminiJSONForModel` (`lib/ai/gemini-provider.ts`) rotates to the next key on a classified HTTP failure (429, 5xx, permission errors) but NOT on its own request timing out. The abort-timeout branch threw `GeminiUnavailableError` immediately, ending the whole call on whichever key happened to be slow -- never trying any keys after it, however many were configured. Key 1 has been quota-exhausted (20/day free tier) for weeks, so every call hits it first, gets a fast 429, and correctly rotates to key 2 -- but if key 2 (or any key after it) takes the full `GEMINI_FETCH_TIMEOUT_MS` (25s) to respond instead of failing fast, the whole call died right there. This is why PDL-088 (raising the key count from 8 to 30) did not fix the outage it was meant to relieve: the rotation logic never reached keys 3 through 10 regardless of how many existed. The code's own comment reasoned "a network-level failure isn't a per-key issue, don't rotate" -- true for a genuine network error (DNS, connection refused), but wrong for OUR OWN configured timeout firing, which says nothing about any other key's health.

**Fix.** A timeout now rotates exactly like a classified failure: log it, try the next key, and only give up once every key has either failed or timed out. If every key timed out and nothing else went wrong, the error now reads "Gemini did not respond within Xms on any of the N key(s)" instead of the misleading original message, which named only the one key that happened to be tried. Two new tests cover it directly: a single timed-out key still rotates to a healthy one and succeeds, and timeouts on every key report unavailability rather than a false quota/suspension verdict.

**University generation, same bug.** `/api/cron/university-generate` calls `generateLesson()`, which goes through the identical `callGeminiJSONForModel` rotation. The runtime error log already showed `[UNIVERSITY_CRON] Supplementary generation failed: Gemini request timed out after 25000ms` three times in the days before this fix -- the near-minute wait the Director saw generating the "Spec-Driven Agent Workflows" lesson was this same stuck-key behaviour, not a separate problem. No separate fix needed; this same change covers it.

**Password reset e-mail: a different system, not yet fixed here.** `/forgot-password` calls `supabase.auth.resetPasswordForEmail()` directly -- Supabase's own built-in auth mail, entirely separate from the app's Resend integration (whose key has been invalid since 2026-09-22, PDL-087, and per the runtime error log is STILL invalid as of 2026-09-25T20:03, after the Director's reported fix -- needs her to check that key was actually saved and redeployed correctly). Confirmed `mulalic71@gmail.com` is a real, active, email/password account (not a look-alike or OAuth-only account), so a reset should have triggered Supabase to send. Could not inspect Supabase's own Auth SMTP settings or delivery logs from here: the Supabase MCP tool in this session authenticates to a different Supabase account entirely (its project list doesn't include this app), and the `SUPABASE_ACCESS_TOKEN` saved in `.env.local` for direct Management API access is now dead (401 Unauthorized on the most basic call, not just for this project -- needs a fresh token from the Supabase dashboard if that direct-access path is still wanted). Best-evidenced guess, not confirmed: Supabase's project-default mailer (used automatically unless custom SMTP is configured under Authentication -> Emails) is documented by Supabase itself as rate-limited and not meant for production; this project has never had custom SMTP configured for Auth specifically, only the app's own separate Resend key for its own notifications. The concrete next step needs the Director's own dashboard access: Supabase Dashboard -> Authentication -> Logs, for the actual attempt around the time she tried it, and -> Authentication -> Emails -> SMTP Settings, to configure a real provider (Resend supports SMTP relay) if the default mailer is confirmed to be the limit.

**Verified.** Typecheck clean, full suite (519 tests, two new) passes.

**Password reset e-mail, resolved (same day, found live with the Director's help).** Two distinct causes stacked on top of each other, found one at a time by triggering a real reset for the Director's own account and reading the Supabase Auth log after each attempt:
1. Supabase's custom SMTP was already enabled, but its saved password was the old, already-dead Resend API key -- `535 "Authentication credentials invalid"`. The Director updated it to the current key (Vercel's own copy of that key was fixed separately, earlier, and is a completely different setting -- the two have to be kept in sync by hand, there is no link between them).
2. With valid credentials, the next attempt failed differently: `550 "This API key is not authorized to send emails from vbj.ai-studio.wiki"`. Resend had only `ai-studio.wiki` (the apex domain) verified, not the `vbj.` subdomain the SMTP sender address used -- domain verification in Resend does not automatically extend to a subdomain. Fixed by pointing the SMTP sender at an address on the already-verified apex domain instead of adding a second domain to verify.
A third real attempt delivered successfully (landed in spam, expected for a domain verified the same day with zero sending history -- will improve as it sends more mail). The Director confirmed the full flow end-to-end: link arrived, password changed.

**A third layer, found triggering a real run right after the rotation fix deployed.** Key rotation now worked (four separate calls each correctly moved from an exhausted key 1 to key 2), but the run still ended `AI-summarized: 0`. The reason: `runEnrichment` (`features/pipeline/enrichment.ts`) processes its two queues strictly in order -- stage 1 (relevance scoring for articles with no score yet) runs to completion or its deadline before stage 2 (summarising already-relevant articles) gets to start at all. With the outage's backlog still at hundreds of unscored articles, stage 1 alone filled the entire enrichment deadline every run, so stage 2 -- the only stage that produces a reportable article -- never got a single second, no matter how many runs passed. The module's own design comment even names the opposite failure it was built to prevent (a pile of unfinished summaries crowding out unscored articles) without covering this direction.

**Fix.** `runEnrichment` now reserves roughly half of whatever time is left when it starts for stage 2, and stage 1's batch loop stops at that earlier cutoff instead of the full deadline. Stage 2 itself still uses the real deadline, so when stage 1 finishes early (the normal case, small backlog) stage 2 gets everything stage 1 left over, not a fixed half. Only the worst case -- a stage-1 backlog large enough to want the whole budget -- is now capped, and only in stage 1's favor by half, never zero.

**Verified.** Typecheck and the full suite (519 tests) pass. A dedicated unit test for `runEnrichment`'s deadline split was not written this session (it needs mocking the full repository/AI-provider layer this function depends on, and the fix was deployed and confirmed against a real production run instead, given how long the report had already been down).
