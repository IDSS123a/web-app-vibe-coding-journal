...
- **S1 (HIGH) Paywall, hold-gate and quiz-answer bypass via direct reads.** Any registered user (free trial is enough) can read through the Supabase REST API: all 77 University lessons ($50 content), all 75 chapter-quiz + 36 level-test questions **including `correct_option_index`**, 55 held-for-review and 5 rejected Daily Reports (defeats the P-6 review gate), all 1000+ articles. Anonymous users read nothing. Proven. No browser-side code reads these tables directly (verified by grep), so the fix is low-risk: drop the authenticated SELECT policies on `daily_reports`, `daily_report_articles`, `articles`, `lessons`, `quiz_questions`, `level_test_questions`, `chapters`, `courses`, `dictionary_terms`, `sources` (server routes use the service role). Re-run the read probe to verify.
...
2. **S1** drop the content SELECT policies + verify with the read probe (small, high value, low risk).
...
6. **Decisions from the Director:** backups (S2), Gemini model migration and key 7 (R2), dashboard empty-state policy (B1), upgrade clock (B7), Assistant caps (B8).
