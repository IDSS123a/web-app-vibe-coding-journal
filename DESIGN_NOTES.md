# DESIGN_NOTES.md — Vibe-Coding Journal / Prompt Hero Studio™

Not yet a Constitution rule (no P-section) — early direction only,
captured 2026-07-18 so it isn't lost before the design pass begins.
...
choice: "ne žurimo nam se."

## Early direction (Director's words, lightly organized)

- Minimalist, black and white, simple — content (the text) should not
  compete with the interface for attention.
- Very modern — should feel current, not dated, using UI patterns
  available today.
- "All advanced functions and options available in UI design today"
  — not yet specified which patterns this means concretely.

## RESOLVED — Design system chosen: Swiss International (International Typographic Style)

Director selected a fully-specified design system on 2026-07-18. This
resolves the "minimalist vs modern/advanced" tension noted above: the
system achieves "modern/advanced" through typographic scale, grid
precision, and mechanical micro-interactions — NOT through visual
density or decorative UI chrome. Minimalism (strict B/W/red palette,
zero shadows, zero rounded corners) and boldness (massive responsive
type, snappy inversions, layered texture patterns) coexist by design.

**Full system prompt is stored verbatim below** — it is already
written as an actionable AI-design-agent instruction (has `<role>`
and `<design-system>` tags), ready to hand to the frontend-design
skill / ACA directly when the design sprint begins. Do not paraphrase
or summarize it away — feed it in full.

### Key tokens (quick reference — see full spec below for everything)
- Palette: `#FFFFFF` background, `#000000` foreground/border, `#F2F2F2`
  muted, `#FF3000` "Swiss Red" as the ONLY accent (functional signal
  only — CTAs, hover, warnings — never decorative fill)
- Typography: Inter, weights 400–900, UPPERCASE headings, extreme
  scale contrast (up to `text-9xl`/`text-[10rem]` desktop headlines)
- Radius: 0px everywhere (strictly rectangular)
- No shadows, no gradients — depth via CSS pattern textures (grid,
  dots, diagonal, noise) at 1.5–4% opacity, never on black or red
- Animation: mechanical, snappy (`duration-150`–`200`, ease-out/
  linear), color inversions not fades, no spring/elastic easing
- Accessibility: built into the spec (21:1 contrast, 44×44px touch
  targets, `prefers-reduced-motion` respected, focus rings in red)

### How to use this when the design sprint starts
1. Feed the full `<role>`/`<design-system>` block below directly to
   whichever ACA runs the design sprint (Claude Code with
   frontend-design skill, or Claude Design).
2. Apply to the ALREADY-BUILT pages first (register, dashboard,
...
## FULL DESIGN SYSTEM SPEC (verbatim, do not edit)
