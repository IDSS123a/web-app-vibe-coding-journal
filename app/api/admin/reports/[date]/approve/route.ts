/**
 * Admin API: PUT /api/admin/reports/[date]/approve
 * Approve a held_for_review report
 * Auth: Admin only
 */
...
export async function PUT(
  ...
  try {
    ...
    const approverEmail = verified.email;
    ...
    const approvedBy = body.approvedBy || approverEmail;
    ...
    await sendAdminNotification({
      subject: `Daily digest for ${date} approved`,
      html: `<p>Report approved by ${approvedBy} at ${now}</p><p>Status: manually_approved</p><p>Articles: ${currentReport.article_count}</p>`,
    });

    // E-6 Step 5: Return
    console.log(`[ADMIN] Report ${date} approved by ${approvedBy}`);
    ...
    console.error(`[ADMIN] Error approving report: ${errorMsg}`);
